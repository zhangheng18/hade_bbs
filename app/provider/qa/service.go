...
func (q *QaService) QuestionsLoadAuthor(ctx context.Context, questions *[]*Question) error {
	...
	if err := q.ormDB.WithContext(ctx).Preload("Author").Find(questions, ids).Error; err != nil {
		return err
	}
	return nil
}
...
func (q *QaService) DeleteAnswer(ctx context.Context, answerID int64) error {
	...
	err := q.ormDB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		// 获取问题
		if err := tx.First(answer).Error; err != nil {
			return err
		}
		question := &Question{ID: answer.QuestionID}
		if err := tx.First(question).Error; err != nil {
...
func (q *QaService) AnswerLoadAuthor(ctx context.Context, answers *Answer) error {
	if err := q.ormDB.WithContext(ctx).Preload("Author").First(answers).Error; err != nil {
		return err
	}
	return nil
}
